...
public class AccountCleanupTask {
    ...
    public void cleanupExpiredAccounts() {
        ...
        while (true) {
            // --- 步骤 A: 查询一批需要删除的 UID ---
            List<Long> idsToDelete = userMapper.selectObjs(
                    // 创建 Lambda 查询包装器，提供类型安全的链式调用
                    new LambdaQueryWrapper<CommonUser>()
                            // select(CommonUser::getUid): 只查询 uid 字段，不查其他字段，减少网络传输和内存占用
                            .select(CommonUser::getUid)
                            ...
                            .and(wrapper -> wrapper
                                    ...
                                    .or(w -> w
                                            ...
                                            .eq(CommonUser::getAccountStatus, AccountStatusEnum.ACCOUNT_STATUS_FORCE_LOGOUT)
            ...
            transactionTemplate.executeWithoutResult(status -> {
                try {
                    ...
                    int count = userMapper.physicalDeleteByUids(idsToDelete);
                    log.info("成功物理删除 {} 个过期/强制销号的普通用户账号", count);
                    totalCleaned.addAndGet(count); // 将本批删除的数量累加到总计数器
                } catch (Exception e) {
                    // 如果删除过程中出错（如数据库连接断开），记录错误日志并抛出异常
                    log.error("批量删除普通用户账号失败，事务回滚。IDs: {}", idsToDelete, e);
